...
class OrgService:
    ...
    def org_link_invite(self, slug: str, role: RoleOptions, request: Request):
        ...
        name = "-".join(name)
        invite_link = (
            f"{request.base_url}{name}/invite/{token}/{role_tok}/mixer=invite/"
        )
        resp = {
    ...
    def join_org(
        ...
        if org_check.revoke_link is False:
            raise HTTPException(
